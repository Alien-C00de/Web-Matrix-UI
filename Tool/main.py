...
async def Main():
    ...
    parser.add_argument('-t', '--timestamp', help='Timestamp value')
    ...
    try:
        if args.Single_Site and args.timestamp and args.Mode:
            url = args.Single_Site.strip()
            timestmp = args.timestamp.strip()
            if not (url.startswith("http://")) and not (url.startswith('https://')):
                url = "https://" + url
            print(f"📡 Searching Details For : {url}", flush=True)
            addr = str(url)
            mode = args.Mode.strip()
            eng = engine.engine(addr, timestmp, mode, False)
