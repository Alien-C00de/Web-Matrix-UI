...
class Summary_Report:
    ...
    async def Generate_Summary_Report(self, website, Server_Location, SSL_Cert, Whois, ser_info, HTTP_sec, headers, cookies, dns_server_info, 
        ...
        body = (
            f"""<body>
                ...
                <div class="ranking-container">
                    ...
                    <h2> <a href="/Tool/output/{Analysis_report}?report={str(percent)}"> {config.ANALYSIS_REPORT_HEADER} </a></h2>
                </div>
                <div class="content">
                    <div class="card">
        ...
        # create and open the new WebMatrix.html file
        if nmap_ops:
            html_report = "%s_%s_%s_%s.html" % (config.REPORT_FILE_NAME, self.domain, config.REPORT_NMAP_FILE_NAME, self.timestamp)
        else:    
            html_report = "%s_%s_%s.html" % (config.REPORT_FILE_NAME, self.domain, self.timestamp)

        BASE_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
        html_report = os.path.join(BASE_DIR, "output", html_report)
        ...
        print(Fore.GREEN + Style.BRIGHT + f"\n📂 HTML" + Fore.WHITE + Style.BRIGHT, filenameH,
                Fore.GREEN + Style.BRIGHT + f"File Is Ready", Fore.RESET)
